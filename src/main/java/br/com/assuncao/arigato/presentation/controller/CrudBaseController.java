...
public abstract class CrudBaseController<T, I, F, S extends ICrudService<T, I, F>> {
    ...
    protected ResponseEntity<List<T>> getAll(@ModelAttribute(FILTER) F filter) {
    ...
    protected ResponseEntity<List<T>> filtro(@ModelAttribute(FILTER) F filter) {
    ...
    protected ResponseEntity<T> editar(@RequestParam(name="id") I i) {
